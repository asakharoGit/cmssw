// Location /CondFormats/JetMETObjects/src
...
//------------------------------------------------------------------------
SimpleMETCorrector::SimpleMETCorrector() 
{ 
  mFunc            = new TFormula(); 
  //mParameters      = new JetCorrectorParameters();
  mParameters      = new METCorrectorParameters();
...
//------------------------------------------------------------------------
/*
SimpleMETCorrector::SimpleMETCorrector(const std::string& fDataFile, const std::string& fOption) 
{
  //mParameters      = new JetCorrectorParameters(fDataFile,fOption);
  mParameters      = new METCorrectorParameters(fDataFile,fOption);
  ...
  std::cout << " SimpleMETCorrector::SimpleMETCorrector(const JetCorrectorParameters& fParameters)" << std::endl; 
}
*/
//------------------------------------------------------------------------
//--- SimpleJetCorrector constructor -------------------------------------
//--- reads arguments from a file ----------------------------------------
//------------------------------------------------------------------------
//SimpleMETCorrector::SimpleMETCorrector(const JetCorrectorParameters& fParameters)
SimpleMETCorrector::SimpleMETCorrector(const METCorrectorParameters& fParameters)
{
  //mParameters      = new JetCorrectorParameters(fParameters);
  mParameters      = new METCorrectorParameters(fParameters);
  //mFunc            = new TFormula("function",((mParameters->definitions()).formula()).c_str());
  //mDoInterpolation = false;
  //if (mParameters->definitions().isResponse())
  //  mInvertVar = findInvertVar();
...
//------------------------------------------------------------------------
/*
float SimpleMETCorrector::correction(const std::vector<float>& fX,const std::vector<float>& fY) const
//float SimpleMETCorrector::correction() const 
{
  // Added by Sasha (begin)
  
  std::cout << " Hello form SimpleJetCorrector::correction " << std::endl;
  for(size_t iz=0; iz<fX.size(); ++iz){
     std::cout << iz << " ) fX = " << fX[iz] << std::endl;
     std::cout << iz << " ) fY = " << fY[iz] << std::endl;  
  }
...
//------------------------------------------------------------------------
/*
float SimpleMETCorrector::correctionBin(unsigned fBin,const std::vector<float>& fY) const 
{
  ...
  for(size_t aai=0; aai<par.size(); ++aai){
     std::cout << " SimpleJetCorrector::correctionBin par= " << par[aai] << std::endl;
  }
...
//------------------------------------------------------------------------
/*
unsigned SimpleMETCorrector::findInvertVar()
{
...
//--- inversion ----------------------------------------------------------
//------------------------------------------------------------------------
float SimpleMETCorrector::invert(const std::vector<float>& fX) const
{
  unsigned nMax = 50;
  unsigned N = fX.size();
